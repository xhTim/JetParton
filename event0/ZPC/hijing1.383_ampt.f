...
clin-9/2023 for jet vn, go directly in front of ZPC:
        if(isoft.eq.4) then
           ...
           write (6,*) 'Need string melting for jet vn studies'
           stop
        endif
...
clin-6/2009:
clin-9/2023 for jet vn, write collision history also for ioscar=1:
c        if(ioscar.eq.3) WRITE (95, *) IAEVT, mul
        if(ioscar.eq.1.or.ioscar.eq.3) WRITE (95, *) IAEVT, mul
...
c     change format for large numbers:
 ...
 1013   CONTINUE
clin-9/2023 increase digits of pT for jet vn:
c 210    format(I6,2(1x,f8.3),1x,f10.3,1x,f6.3,4(1x,f8.2))
c 211    format(I6,2(1x,f8.3),1x,f10.3,1x,f6.3,4(1x,e8.2))
 210    format(I6,3(1x,f9.3),1x,f6.3,4(1x,f8.2))
 211    format(I6,3(1x,f9.3),1x,f6.3,4(1x,e8.2))
...
clin-9/2023 for jet vn, read shower particles as initial condition:
 275    call readShower
...
clin-9/2023 for jet vn, skip hadronization and hadron cascade for now:
        if(isoft.eq.4) return
